...
if($dopost!='save')
{
    ...
    $keywords = $writer = $source = $body = $description = $title = $original = $type = $item = $inner = $outner = $thickness = $weight = '' ;
...
else if($dopost=='save')
{
    ...
    $templet = empty($templet) ? '' : $templet;
     // $query = "INSERT INTO `{$addtable}`(aid,typeid,redirecturl,templet,userip,body{$inadd_f},original,type,item,inner,outner,thickness,weight) Values('$arcID','$typeid','$redirecturl','$templet','$useip','$body'{$inadd_v},'$original','$type','$item','$inner','$outner','$thickness','$weight')";
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        $dsql->ExecuteNoneQuery("Delete From `#@__archives` where id='$arcID'");
        $dsql->ExecuteNoneQuery("Delete From `#@__arctiny` where id='$arcID'");
        ShowMsg("把数据保存到数据库附加表 `{$addtable}` 时出错，请把相关信息提交给DedeCms官方好吧。".str_replace('"','',$gerr),"javascript:;");
        exit();
    }
    //生成HTML
